...
def decode_packets(frame: bytes, token_to_contract: dict[int, str], received_at: datetime | None = None):
    ...
    for _ in range(count):
        ...
        if token in token_to_contract: result.append(Quote(token_to_contract[token],received_at,ltp,bid,ask,struct.unpack_from(">I",packet,20)[0] if length>=24 else 0,struct.unpack_from(">I",packet,48)[0] if length>=52 else None,depth))
    return result
