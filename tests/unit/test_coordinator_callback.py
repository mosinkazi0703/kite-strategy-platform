...
def test_underlying_callback_uses_late_start_reference(tmp_path):
    c=PaperTradingCoordinator("x","y",tmp_path); c.rows=[{"exchange":"NSE","tradingsymbol":"NIFTY","instrument_token":"1","expiry":"","strike":"","instrument_type":"EQ","lot_size":"1","tick_size":".05","name":"NIFTY"}]; c.subscribe_underlying_first()
    assert c.on_underlying_quote(Quote("1",datetime(2026,1,1,4,0,tzinfo=timezone.utc),last=20100))==20100
    assert c.reference_kind=="late_start_reference"
