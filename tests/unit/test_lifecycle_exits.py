...
def test_target_exit_persists_trade(tmp_path):
 ...
 q={k:Quote(k,datetime.now(timezone.utc),last=0,bid=0,ask=0) for k in q}; l.mark(q,datetime.now(timezone.utc)); l.mark(q,datetime.now(timezone.utc)); assert p.state=="CLOSED" and len(s.ledger.rows)==1
